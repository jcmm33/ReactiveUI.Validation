...
namespace ReactiveUI.Validation.Droid
{
    public class SampleViewModel:ReactiveObject,IValidateable
    {
        ...
        public SampleViewModel()
        {
            ...
            this.Validator(vm => vm.Name, name => name.Length > 3, "You must specify a valid name");

            // age must be between 13 and 100, message includes the silly length
            this.Validator(vm => vm.Age, age => age >= 13 && age <= 100, "{0} is a silly age");

            // age must be greater than 10 and shoe size > 4 for a valid model
            this.Validator(this.WhenAny(vm => vm.Age, vm => vm.ShoeSize, 
            ...
            Save = ReactiveCommand.CreateAsyncTask(ValidationContext.Valid, async _ =>
            {
                // some save code here
            });
