...
namespace ReactiveUI.Validation
{
    public static class ViewModelIValidateableMixins
    {
        ...
        /// <returns></returns>
        /// <remarks>
        /// It should be noted that the observable should provide an initial value, otherwise that can result in an inconsistent performance.</remarks>
        public static ValidationHelper ValidationRule<TViewModel>(this TViewModel viewModel,Func<TViewModel,IObservable<bool>> validationObservableFunc,Func<TViewModel,bool,string> messageFunc)
            where TViewModel : ReactiveObject, ISupportsValidation
        {
            var validation = new ModelObservableValidation<TViewModel>(viewModel,validationObservableFunc,messageFunc);
